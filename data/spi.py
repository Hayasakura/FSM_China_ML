"""
Script: Stream Power Index (SPI)
Data Sources:
- DEM: NASA SRTM (USGS/SRTMGL1_003)
- Upstream area: MERIT-Hydro
Formula: SPI = ln(As * tan(slope))
"""
...
def calculate_spi(dem, upa):
    ...
    slope_rad = ee.Terrain.slope(dem).multiply(3.141592 / 180.0)
    ...
    spi = As.multiply(tan_slope).max(eps).log().rename("SPI")
    
    return spi.clip(china)
...
def main():
    ...
    # Calculate SPI
    spi = calculate_spi(dem, upa)
    
    # Export
    print("Exporting SPI data (1km resolution, CRS=EPSG:4326)...")
    task = export_to_drive(
        image=spi,
        description='china_spi_NASA_MERITHydro_1km',
